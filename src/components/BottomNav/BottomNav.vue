    ...
    <nav class="w-full bg-white">
     <div class="navbar__container">
        ...
        <div
          ...
          @click="toggleCreate"
        >
          <div v-show="!showCreate" class="plus mx-auto leading-tight z-50 text-2xl text-white">+</div>
          <div v-show="showCreate" class="plus mx-auto leading-tight z-50 text-2xl text-white">-</div>
        </div>
        <div class="navbar__icon">
     ...
     <!-- Hidden Navigation -->
      <div v-show="showCreate" class="w-full bg-white flex flex-col p-4 navigation__hidden">
        <div
          v-if="displayNoteButton"
          @click="goToRoute(noteRoute)"
          style="height: 40px; width: 160px"
          class="bg-red-500 text-white mx-auto py-2 px-4 my-4 text-center rounded-lg"
        >ADD NOTE</div>
        <div
          style="height: 40px; width: 160px"
          class="bg-gray-900 text-white mx-auto py-2 px-4 my-4 text-center rounded-lg"
    ...
    noteRoute () {
      if (this.$route.path.startsWith("/stack")) {
        return {
          path: "/create/note",
          query: {
            stack: this.$route.params.id,
            lastRoute: this.$route.fullPath
          }
        }
      } else {
